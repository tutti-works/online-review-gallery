...
async function processFile(importJobId, fileId, fileName, fileType, studentName, studentEmail, galleryId, auth) {
    ...
    try {
        ...
        // 完了チェックを実行
        await (0, importController_1.checkImportCompletion)(importJobId);
...
async function processImageFile(imageBuffer, fileName, storage, galleryId) {
...
async function processPdfFile(pdfBuffer, fileName, storage, galleryId) {
    const bucket = storage.bucket();
