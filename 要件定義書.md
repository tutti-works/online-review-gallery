...
* **2025-10-07 (3):** PDF変換をJPEG経由に修正（メモリ使用量削減によるSegmentation Fault対策）、エラー詳細記録機能追加、インポートUI改善
* **2025-10-07 (4):** 複数ファイル提出時のサムネイル重複生成バグを修正（全体で1ページ目のみサムネイル生成）、リダイレクト時のbeforeunload警告を無効化
* **2025-10-07 (5):** 画像ファイル処理時のサムネイル重複生成バグを修正（PDF同様、複数ファイル提出時に1ページ目のみサムネイル生成）
