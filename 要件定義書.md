...
* **F-02-03: 提出物の変換とアップロード**
  ...
  * **PDFファイルの場合:** PDFの各ページを、それぞれ個別のJPG画像（3400x2404px、A3横向き比率）に変換する。Cloud Runを使用してGraphicsMagick/Ghostscriptでの高品質変換を行う。
  * サムネイル画像（420x297px）を自動生成し、アスペクト比を維持する。
...
* 🔄 **ハイブリッド方式によるギャラリー管理**
  * `galleries`コレクションでギャラリーのメタデータを管理
  * `artworks`コレクションに`galleryId`フィールドを追加してフィルタリング
  * ギャラリー切替UI（ヘッダーのドロップダウン）
  * ギャラリーごとのデータリセット機能
...
* **2025-10-06:** 複数ファイル提出の統合処理実装（同じ学生の複数ファイルを1つのartworkにまとめる）
...
```
galleries (コレクション) ← メタデータ管理
├── gallery_id (ドキュメント)
│   ├── id: string
│   ├── courseName: string （授業名）
│   ├── assignmentName: string （課題名）
│   ├── courseId: string
│   ├── assignmentId: string
│   ├── artworkCount: number （キャッシュ）
│   ├── createdAt: Timestamp
│   └── updatedAt: Timestamp

artworks (コレクション) ← 全作品データ
├── artwork_id (ドキュメント)
│   ├── galleryId: string ← 追加（どのギャラリーに属するか）
│   ├── （その他のフィールドは現在と同じ）
│   └── ...
