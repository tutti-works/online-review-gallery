...
- **F-02-07: 再インポートスキップ機能** 🔄 **実装予定**
  - 同一課題の再インポート時、既に作品が存在する学生の処理をスキップする。
  - 判定キーは `galleryId + studentEmail` の組み合わせ。
  - スキップされた学生数をインポート完了時に表示する。
  - 処理エラーで失敗した学生は、再インポートで再処理される。
- **F-02-08: 未提出学生のプレースホルダー作品** 🔄 **実装予定**
...
- **F-03-04: 並び替え・フィルタリング**
  ...
  - **未提出/エラー作品フィルター（実装予定）:** 🔄 チェックボックスで未提出・エラー作品を非表示にできる
  - **特殊な並び替えロジック（実装予定）:** 🔄
...
- **2025-11-06:** ギャラリー作品数同期機能実装、artworks配列フィールド非推奨化
