...
- 未ログインで `/showcase` 配下のURLへ直接アクセスした場合、現在のパス・クエリ・ハッシュを `next` パラメータとして専用ログインページへ引き継ぐ。
- ログイン成功後は `next` で指定された元のShowcaseページへ戻る。`next` がない場合は入口ページ `/showcase` を表示する。
- オープンリダイレクトを防ぐため、戻り先には同一サイト内の `/showcase` 配下のみを許可する。外部URL、不正なURL、または `/showcase/login` 自身が指定された場合は `/showcase` へフォールバックする。
- ログイン後の遷移には履歴を残さない `replace` を使用する。
...
## Change Log
- 2026-07-22: Showcaseの課題詳細URLなどへ未ログインで直接アクセスした場合、専用ログイン後に元のURLへ戻る導線を追加。戻り先は同一サイトの `/showcase` 配下に制限し、不正な指定は入口ページへフォールバックする。
