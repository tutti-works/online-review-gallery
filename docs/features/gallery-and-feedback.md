...
| 機能ID | 機能名 | 説明 |
|--------|--------|------|
| F-03 | ギャラリー表示機能 | レスポンシブCSS Grid、モーダル表示、ズーム、ソート、フィルター |
| F-04 | 評価・フィードバック機能 | いいね、コメント、ラベル、作品削除 |
| F-05 | データ管理機能 | 全データリセット、ギャラリー別削除、作品数同期 |

### 授業アーカイブ（Issue #11、未デプロイ）

ダッシュボードの「授業アーカイブ管理」で授業を選び、確認後にアーカイブまたは解除できます。通常の `/gallery` には現役授業、`/archive` にはアーカイブ済み授業が表示されます。どちらも授業から課題を選び、同じ作品閲覧・評価・注釈機能を利用できます。

アーカイブ状態は `archivedCourses/{courseId}` の有無で判定します。作成時の `archivedAt` はサーバー時刻です。記録のない既存授業は現役として扱い、同じ `courseId` の課題はまとめて切り替わります。Gallery・Artwork・Storage・Showcase のデータは移動・変更しません。課題データ削除は別の操作です。通常とアーカイブの最終閲覧課題は別々に保存します。

---

## 2. ギャラリー表示機能 (F-03)

### 2.1. レスポンシブCSS Grid表示 (F-03-01)
...
**個別ラベルフィルタリング**:
- 赤・青・緑（各1〜5）のうち、現在のGallery全作品に実在する評価だけを表示する
- 複数選択はOR条件。絞り込み結果によって候補は減らない

**合計ラベルフィルタリング** ✅ **実装済み**:
- 入力済みの各色の点数を加算し、現在のGallery全作品に実在する合計点だけを表示する（最大15点、1点も有効）
- プルダウン選択で合計値を指定（デフォルト: "合計で絞り込み"）
- 合計フィルター選択時は個別ラベルボタンを自動的に無効化（`opacity-50 cursor-not-allowed`スタイル適用）
- 合計フィルター解除時は個別ラベルボタンを再び有効化
- 未入力の色は0相当。不正な同色複数評価があっても、色ごとに配列末尾の有効な点数だけを集計する
...
**概要**: 講評会で作品や評価帯を選んで表示するための補助評価。成績簿や正式採点ではない

**仕様**:
- 赤・青・緑（各1〜5）の計15種類。色は特定の教師に固定せず、管理者はどの色でも入力できる
- 全色の入力は必須ではなく、各色0〜1個を選択する。同色の別点数を押すと置換し、選択中の点数を再度押すと解除する
- 他色の評価は維持する。個別評価と合計評価は排他的な閲覧モードとして扱う
- ラベルは作品サムネイルとモーダルに表示

**実装**:
- Firestoreの既存`labels: LabelType[]`配列を維持し、データ移行は行わない
- 更新時はtransactionで最新の配列を読み、対象色だけを変更する。近接した複数管理者の更新でも他色を上書きしない
- Showcaseも同じ`labels`配列を読み込み・表示する
