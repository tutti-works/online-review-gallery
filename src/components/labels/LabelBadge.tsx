...
const LabelBadge = ({ label, className = '', isActive = true }: LabelBadgeProps) => {
  ...
  return (
    <span title={label} aria-label={`${label.split('-')[0]} ${definition.symbol}点`} className={`inline-flex h-5 w-5 items-center justify-center text-xs font-bold ${colorClass} ${className}`}>
      {definition.symbol}
    </span>
  );
};
